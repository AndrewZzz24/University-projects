...
struct hash_set {
    ...
    string NEXT_KEY = "none";
    string PREV_KEY = "none";
} hash_set1[1000000];

void prev1(string &key);

void next1(string &key);
...
void next1(string &key) {
    ...
    if (pointer != nullptr) {
        if (get1(key)->NEXT_KEY != "none") cout << get1(get1(key)->NEXT_KEY)->value << endl;
        else cout << get1(key)->NEXT_KEY << endl;
    } else cout << "none" << endl;
}
...
void prev1(string &key) {
    hash_set *pointer = get1(key);
    if (pointer != nullptr) {
        if (pointer->PREV_KEY != "none") cout << get1(pointer->PREV_KEY)->value << endl;
        else cout << get1(key)->PREV_KEY << endl;
    } else cout << "none" << endl;
}
...
void delete1(string &key) {
    if (exists1(key)) {
        ...
        while (pointer != nullptr) {
            if (pointer->key == key) {
                if (pointer->NEXT_KEY!="none") get1(pointer->NEXT_KEY)->PREV_KEY=pointer->PREV_KEY;
                if (pointer->PREV_KEY!="none") get1(pointer->PREV_KEY)->NEXT_KEY=pointer->NEXT_KEY;
                if (pointer->key==last_key) last_key=pointer->PREV_KEY;
...
void put1(string &key, string &value) {
    if (!exists1(key)) {
        ...
        a->key = key;
        long long result = hash_function(key);
        if (last_key.empty()) { last_key = key; }
        else {
            if (get1(last_key) != nullptr) {
                get1(last_key)->NEXT_KEY = key;
                a->PREV_KEY = last_key;
            }
            //a->PREV_KEY = last_key;
        }
...
int main() {
    ...
    while (cin >> command) {
        ...
        } else if (command == "prev") prev1(key);
        else if (command == "next") next1(key);
    }
}